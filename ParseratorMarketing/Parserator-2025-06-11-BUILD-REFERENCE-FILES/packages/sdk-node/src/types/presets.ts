...
export const EMAIL_PARSER: ParsePreset = {
  name: 'Email Parser',
...
export const INVOICE_PARSER: ParsePreset = {
  name: 'Invoice Parser',
...
export const CONTACT_PARSER: ParsePreset = {
  name: 'Contact Parser',
...
export const CSV_PARSER: ParsePreset = {
  name: 'CSV Parser',
...
export const LOG_PARSER: ParsePreset = {
  name: 'Log Parser',
...
export const DOCUMENT_PARSER: ParsePreset = {
...
export const ALL_PRESETS: Record<string, ParsePreset> = {
  EMAIL_PARSER,
  INVOICE_PARSER,
  CONTACT_PARSER,
  CSV_PARSER,
...
// Helper functions
export function getPresetByName(name: string): ParsePreset | undefined {
  return Object.values(ALL_PRESETS).find(preset => preset.name === name);
}

export function getPresetsByTag(tag: string): ParsePreset[] {
  return Object.values(ALL_PRESETS).filter(preset => 
    preset.examples.length > 0 && preset.name.toLowerCase().includes(tag.toLowerCase())
  );
}

export function listAvailablePresets(): string[] {
  return Object.values(ALL_PRESETS).map(preset => preset.name);
}
